...
export function buildDesign(prompt) {
  ...
  const tags = genre === 'memory' ? ['记忆', '谜题', 'Agent生成'] : genre === 'reaction' ? ['反应', '街机', 'Agent生成'] : genre === 'quiz' ? ['问答', '剧情', 'Agent生成'] : ['冒险', '选择', 'Agent生成'];
  const summaryMap = {
    memory: '一个根据创作者意图生成的记忆挑战，并以可移植 Web 产物交付。',
    reaction: '一个带分数、计时和可重玩规则的快速反应循环，由本地 Agent 编排器生成。',
    quiz: '一个带清晰状态转换和确定性胜利条件的分支问答体验。',
    adventure: '一个选择驱动的互动冒险，可预览、发布，并从对象存储中运行。'
  };
...
<main class="game-shell">
  <header class="topbar">
    <div><div class="eyebrow">AI Agent 横版游戏 · 对象存储运行时</div><h1 class="title">${safeTitle}</h1><p class="summary">${safeSummary}</p></div>
